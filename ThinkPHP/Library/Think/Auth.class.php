...
class Auth{
    ...
    public function getGroups($uid) {
        ...
        $user_groups = M()
            ...
            ->field('user_id uid,group_id,title,rules')->select();
        $groups[$uid]=$user_groups?:array();
        return $groups[$uid];
    }

    // 自己用
    public function authListh($uid, $type=1) {
        //读取用户所属用户组
        ...
        //读取用户组所有权限规则
        $rules = M()->table($this->_config['AUTH_RULE'])->where($map)->field('id,pid,title,name,condition')->select();
        //print_r($rules);
        return $rules;
    }
